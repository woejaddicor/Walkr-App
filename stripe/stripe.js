...
const STRIPE_PUBLISHABLE_KEY =
  "pk_test_51JtryzDI7uqweVwSdrexArAX6viQxKglm5EMTZFWpQPtxtEnSvbniZoLhjiHnMFlin4MarjsjrpNwmQNio5GuHyk00Nl990MFX";
let currentUser = {};
let customerData = {};
    ...
    .onSnapshot((snapshot) => {
      snapshot.forEach((doc) => {
        ...
        if (payment.status === "new" || payment.status === "requires_confirmation") {
          ...
          content = `🚨 Payment for ${formatAmount(payment.amount, payment.currency)} ${
            payment.status
          }`;
          handleCardAction(payment, doc.id);
        } else {
          content = `⚠️ Payment for ${formatAmount(payment.amount, payment.currency)} ${
            payment.status
          }`;
        }
...
async function handleCardAction(payment, docId) {
  const { error, paymentIntent } = await stripe.handleCardAction(payment.client_secret);
  if (error) {
    alert(error.message);
